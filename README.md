...
> Сповіщення створюються асинхронно після лайка поста месседж якому відправляється через taskiq в RabbitMQ
> Щоб отримати запис у цьому ендпоінті, користувач **B** має лайкнути пост користувача **A**(а перед цім користувач **А** має його створити і потом за допомогою jwt перейти на користувача **В**), після чого **A** викликає >`GET /notifications/`. 

`GET /notifications/`
- Auth: потрібен
- Що повертає: тільки нотифікації поточного користувача (найновіші зверху)
- Body: не потрібен
- Response `200`:
```
[
